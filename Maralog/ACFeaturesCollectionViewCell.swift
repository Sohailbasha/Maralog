...
//    }
    ...
    func changeCellAsthetic(color1: UIColor, color2: UIColor, radius: CGFloat, isOnValue: Bool) {
        setting?.isOn = isOnValue
        UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: [], animations: {
            self.layer.cornerRadius = radius
            self.backgroundColor = color1
            self.settingIcon.tintColor = color2
            self.settingNameLabel.textColor = color2
        }, completion: nil)
    }
